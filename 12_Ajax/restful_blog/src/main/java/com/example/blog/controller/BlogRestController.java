...
//    @Autowired
//    BlogServiceImpl blogServiceRest;
    @Autowired
    ...
    public Page<Blog> showList(Model model, @RequestParam(value = "page", defaultValue = "1") int page){
        ...
        return blogService.findAll(PageRequest.of(page, 5));
    }
    ...
    @GetMapping("/search")
    public Page<Blog> search(@RequestParam("nameSearch") String name, Pageable pageable, Model model) {
//        return (ResponseEntity<Blog>) blogService.search(name, PageRequest.of(0, 5));
        return blogService.searchRestBlog(name,PageRequest.of(0, 5));
    }
}
